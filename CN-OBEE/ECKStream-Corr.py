import Undirected_graph_CN
import pandas as pd
import communication_CN
import numpy as np
from sklearn.metrics import mean_squared_error
from sklearn.metrics import mean_absolute_error

G, W = Undirected_graph_CN.graph()
V = G.nodes()
E = G.edges(data=True)
...
def Event_triggered(epsilon):
    ...
    for node in V:
        ...
        communication_CN.send_threshold(G, node, Threshold[node], U)

    node_to_index = {node: idx for idx, node in enumerate(V)}
    first_trigger = {node: True for node in V}
    ...
    last_trigger_time = {node: 0 for node in V}
    pre_trigger_time = {node: 0 for node in V}
    last_neighbor_influence = {node: 0.0 for node in V}
    ...
    for index, row in file.sort_values(by='time').iterrows():
        ...
        for node in V:
            ...
            for neighbor in G.neighbors(node):
                if (last_trigger_time[neighbor] in U[node] and
                        'ave_value' in U[node][last_trigger_time[neighbor]] and
                        neighbor in U[node][last_trigger_time[neighbor]]['ave_value']):
                    neighbor_latest_ave_value = U[node][last_trigger_time[neighbor]]['ave_value'][neighbor]
                    neighbor_last_value = U[node][last_trigger_time[neighbor]]['last_value'][neighbor]
                    neighbor_std_dev = U[node][last_trigger_time[neighbor]]['std_dev'][neighbor]
                    weight = W[node_to_index[node], node_to_index[neighbor]]
                    neighbor_influence += weight * abs(neighbor_last_value - neighbor_latest_ave_value)
                    neighbor_std += weight * neighbor_std_dev
            ...
            if abs_value + neighbor_influence > 1.0 * std + 1.0 * neighbor_std:
                if (first_trigger[node] or abs_value > 1.0 * std or
                        neighbor_influence != last_neighbor_influence[node]):
                    ...
                    values = [info['value'] for info in information_list]
                    ave_value = round(pd.Series(values).mean(), 2)
                    last_value = values[-1]
                    std_dev = round(pd.Series(values).std(), 2) if len(values) > 1 else 0.0

                    if trigger_times[node] == 1:
                        pre_trigger_time[node] = 0
                        last_trigger_time[node] = current_time
                    else:
                        pre_trigger_time[node] = last_trigger_time[node]
                        last_trigger_time[node] = current_time

                    communication_CN.send_information(node, information_list, ave_value, last_value, std_dev,
                                                      current_time,
                                                      G, U)
                    ...
                    first_trigger[node] = False
                    last_neighbor_influence[node] = neighbor_influence
                    x[node] = [trigger_time_value[node]]

            else:
                first_trigger[node] = True
    ...
    for node in V:
        ...
        communication_CN.send_information(node, information_list, ave_value, last_value, std_dev, current_time,
                                          G, U)
    return U, Threshold
...
def main():
    ...
    with open("results_set.txt", "a") as f:
